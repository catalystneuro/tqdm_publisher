...
def _run_sleep_in_subprocess(args: Tuple[int, int]):
    ...
    sub_progress_bar = TQDMPublisher(
        iterable=iterable, position=iteration_index + 1, desc=f"Progress on iteration {iteration_index} ({id})", leave=False
    )
    ...
    def run(self, number_of_jobs: int, repeats: List[int]):
        ...
        with ProcessPoolExecutor(max_workers=number_of_jobs) as executor:
            ...
            job_map = executor.map(
                _run_sleep_in_subprocess, [(repeat, iteration_index, uuid.uuid4(), PORT) for iteration_index, repeat in enumerate(repeats)]
            )
